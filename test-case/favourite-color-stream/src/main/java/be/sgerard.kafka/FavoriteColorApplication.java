...
public class FavoriteColorApplication {
    ...
    private static Topology createTopology(){
        ...
        builder.<String, String>stream("favorite-user-color")
                .filter((k, v) -> v.contains(","))
                .map((k, v) -> new KeyValue<>(v.split(",")[0], v.split(",")[1]))
                .toTable()
                .groupBy((key, value) -> new KeyValue<>(value, value))
                .count(Materialized.as("favorite-user-color"))
                .toStream()
                .to("favorite-user-color-stats");
